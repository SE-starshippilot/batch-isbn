...
def getBookInfo(currentRow:pd.Series)->list:
    ...
    if  (conf.window.metadata['append'] and currentRow.isna().sum() > 2 + len(conf.EXCEL_FIELDS)) or \
        ...
        raise MissingInfoError('Missing information in the input row.', currentRow.to_dict())
    ...
    if bookPage['numFound']:
        ...
        for attr in conf.QUERY_FIELDS: # check if title and author matches
            ...
            if retAttr is None or isWrongInfo(retAttr, currentRow[conf.QUERY_2_EXCEL[attr]]): 
                raise MissingInfoError(f"the retrived book's {attr} {retAttr} doesn't match with {currentRow[conf.QUERY_2_EXCEL[attr]]}", currentRow.to_dict())
        ...
        if not(editionInfo.get('ISBN')):
            raise MissingInfoError('Found book, but Open Library returns no ISBN.', currentRow.to_dict())
        return {**bookInfo, **editionInfo}
    raise MissingInfoError(f'No book found for {currentRow["Title"]}', currentRow.to_dict())
